...
void output_account_balances_to_file(int total_number_of_accounts, float account_balances[]) {
  FILE *account_balances_file_pointer;
  account_balances_file_pointer = fopen("account_balances.output", "w");
  int balances_display_counter = 0;
  fprintf (account_balances_file_pointer, "==== List of Account Balances ====\n");
  while (balances_display_counter < total_number_of_accounts) {
    fprintf (account_balances_file_pointer, "%3d: $%8.2f\n", balances_display_counter, account_balances[balances_display_counter]);
    balances_display_counter++;
  }
  fclose(account_balances_file_pointer);
}