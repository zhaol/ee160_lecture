...
#include "helper_functions_with_fee_without_passing_array.h"
#include "helper_functions_with_fee_without_passing_array.c"
...
int main() {
  ...
  while (exit_program_flag == CONTINUE_PROGRAM) {
    ...
    if (account_type == 's') {
      minimum_balance_fees[current_account_number] = is_account_below_minimum_balance(
        account_balance,
        SAVINGS_ACCOUNT_MINIMUM_BALANCE,
        &minimum_balance_fee_charged_flag); // pass minimum_balance_fee_charged_flag to function by reference
    } else {
      minimum_balance_fees[current_account_number] = is_account_below_minimum_balance(
        account_balance,
        CHECKING_ACCOUNT_MINIMUM_BALANCE,
        &minimum_balance_fee_charged_flag); // pass minimum_balance_fee_charged_flag to function by reference
    }  
  ...
  int account_display_counter = 0;
  printf("=== List of Account Balances ===\n");
  while (account_display_counter < current_account_number) {
    printf("$%.2f, $%d\n", account_balances[account_display_counter], minimum_balance_fees[account_display_counter]);
    account_display_counter++;
  }  
