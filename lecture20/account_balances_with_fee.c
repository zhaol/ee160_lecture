...
#include "helper_functions_with_fee.h"
#include "helper_functions_with_fee.c"
...
int main() {
  ...
  while (exit_program_flag == CONTINUE_PROGRAM) {
    ...
    if (account_type == 's') {
      minimum_balance_fee_charged_flag = is_account_below_minimum_balance(
        account_balance,
        SAVINGS_ACCOUNT_MINIMUM_BALANCE,
        minimum_balance_fees, // arrays can be viewed as pointers to the first array element
