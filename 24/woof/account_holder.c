...
int main() {
  ...
  //file_for_account_summary();
  FILE *summary_file; 
 summary_file = fopen("file_for_account_summary", "w");
 
  // Display accounts summary information
  fprintf ("==== Accounts Summary ====\n");
  fprintf ("The total number of accounts: %d\n", total_number_of_accounts);
  fprintf ("The total amount in all accounts: $%.2f\n", total_amount_in_accounts);
  fprintf ("The percentage of the total minimum balance fees with respect to the total account balances: %.1f%%\n", total_minimum_balance_fees / total_amount_in_accounts * PERCENT_CONVERSION_FACTOR);
  fprintf ("The percentage of accounts that were charged a minimum balance fee: %.1f%%\n", PERCENT_CONVERSION_FACTOR * total_accounts_with_minimum_balance_fees / total_number_of_accounts);
