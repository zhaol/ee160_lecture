...
void output_list_of_account_balances(float account_balances[][MAX_NUMBER_OF_ATTRIBUTES], int number_of_accounts) {
  ...
  if (file_pointer == NULL) {
    ...
    while (balances_display_counter <= number_of_accounts) {
      fprintf (file_pointer, "%3d: $%8.2f, $%8.2f\n",
        ...
        account_balances[balances_display_counter+ARRAY_OFFSET][STARTING_ACCOUNT_BALANCE+ARRAY_OFFSET],
        account_balances[balances_display_counter+ARRAY_OFFSET][ENDING_ACCOUNT_BALANCE+ARRAY_OFFSET]);
      balances_display_counter++;
    }  
    fclose(file_pointer);
    ...
    int index;
    for(index=0;index<=balances_display_counter;index++){
      int index2;
      for(index2=0;index2<=1;index2++) {
        printf("[%d][%d]: %f ", index, index2, account_balances[index][index2]);
      }
      printf("\n");
    }
  }
