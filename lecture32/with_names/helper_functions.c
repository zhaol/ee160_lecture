...
// need to pass in list of names
void output_list_of_account_balances(
  float account_balances[][MAX_NUMBER_OF_COLUMNS],
  ...
  if (file_pointer == NULL) {
    ...
    while (balances_display_counter <= number_of_accounts) {
      ...
      // print out with list of names
      fprintf (file_pointer, "%3d: $%8.2f $%8.2f\n",
        balances_display_counter,
        account_balances[balances_display_counter+ARRAY_OFFSET][STARTING_BALANCE+ARRAY_OFFSET],
        account_balances[balances_display_counter+ARRAY_OFFSET][ENDING_BALANCE+ARRAY_OFFSET]);
