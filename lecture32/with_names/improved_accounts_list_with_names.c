//https://github.com/zhaol/ee160/tree/gh-pages/code/improved_accounts_list_with_name
...
int main() {
  ...
  //float account_balances[MAX_NUMBER_OF_ACCOUNTS]; // 1D array
  float account_balances[MAX_NUMBER_OF_ACCOUNTS][MAX_NUMBER_OF_COLUMNS] = {0}; // 2D array
  ...
  do  {
    ...
    if (valid_account_type_flag == VALID_ACCOUNT_TYPE) {
      ...
      if (minimum_balance_fee_charged_flag) {
        ...
        account_balances[current_account_number+ARRAY_OFFSET][ENDING_BALANCE+ARRAY_OFFSET] = account_balance - minimum_balance_fee;
      } else {
        printf ("Minimum Balance Fee to Account Balance Percentage: %.1f%%\n", NO_MINIMUM_BALANCE / account_balance);
        printf ("The new account balance is $%.2f\n", net_balance(account_balance, NO_MINIMUM_BALANCE));
        account_balances[current_account_number+ARRAY_OFFSET][ENDING_BALANCE+ARRAY_OFFSET] = account_balance - NO_MINIMUM_BALANCE;
      }
      
      update_account_summary_information(&total_number_of_accounts, &total_amount_in_accounts, account_balance);
      account_balances[current_account_number+ARRAY_OFFSET][STARTING_BALANCE+ARRAY_OFFSET] = account_balance;
      
      /*
      int debug, debug2;
      for(debug=0; debug<=10; debug++) {
        for(debug2=0;debug2<=1;debug2++) {
          printf("debug: %f\n", account_balances[debug][debug2]);
        }
        printf("\n");
      }
      */
