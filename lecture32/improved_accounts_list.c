...
int main() {
  ...
  do  {
    ...
    if (valid_account_type_flag == VALID_ACCOUNT_TYPE) {
      ...
      account_balances[current_account_number+ARRAY_OFFSET][STARTING_ACCOUNT_BALANCE+ARRAY_OFFSET] = account_balance;
      account_balances[current_account_number+ARRAY_OFFSET][ENDING_ACCOUNT_BALANCE+ARRAY_OFFSET] = net_balance(account_balance, minimum_balance_fee);
    }
